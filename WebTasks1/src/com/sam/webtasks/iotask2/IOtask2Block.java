...
public class IOtask2Block {
	...
	public void Run() {
		if (targetValues.size() == 0) { // no target values have been set up
			if (nTrials==1) {
				//if there's only 1 trial, we assume that there's no need to ask a pre-trial question
				targetValues.add(-1); //-1 instructs the code not to ask a pre-trial question
			} else if (nTrials==17) {
				//17 trials is the standard number of trials for 4 of each forced condition plus 9 choice trials
				...
				//now set up the values corresponding to the forced internal / forced external conditions
				int forcedA = 0; //begin with forced internal
				int forcedB = 10; //then have forced external
				
				if(Counterbalance.getFactorLevel("forcedOrder")==1) { //depending on counterbalancing version, switch these around
					forcedA = 10;
					forcedB = 0;
				}
	...
	//the variables below are used during trials of the task. they don't need to be set up
